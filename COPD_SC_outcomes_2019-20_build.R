...
filename <- "C:/Users/aadamson/Documents/COPD/SC_outcomes_2019-20/Analysis/Logs/COPD_SC_Outcomes_2020_build_log"
filedate <- Sys.Date()
...
dat <- readstata13::read.dta13("C:/Users/aadamson/Documents/COPD/SCC 2020/COPD_SCC_2020_build.dta",
                                   generate.factors = TRUE)


checkSame(dat, dat_old)

dat <- dat %>% rename(lsoa11 = lsoa11cd)

table(dat$news2, useNA = "ifany")
table(dat$q3_1firstrecordednewsscore, useNA = "ifany")

dat <- dat %>% select(-warningsaccepted)
...
HES1 <- read.csv("C:/Users/aadamson/Documents/COPD/SC_outcomes_2019-20/Data/rawData/FILE0154340_NIC349273_HES_APC_E_201999.txt",
                     sep = "|", stringsAsFactors = FALSE)

# write.csv(HES1, "D:/Alex/COPD/SC Outcomes 2018/data/rawData/viewing/HES1.csv", row.names = FALSE) 

HES2 <- read.csv("C:/Users/aadamson/Documents/COPD/SC_outcomes_2019-20/Data/rawData/FILE0154341_NIC349273_HES_APC_E_202099.txt",
                     sep = "|", stringsAsFactors = FALSE)

HES3 <- read.csv("C:/Users/aadamson/Documents/COPD/SC_outcomes_2019-20/Data/rawData/FILE0154342_NIC349273_HES_APC_W_201899.txt",
                 sep = "|", stringsAsFactors = FALSE)

HES4 <- read.csv("C:/Users/aadamson/Documents/COPD/SC_outcomes_2019-20/Data/rawData/FILE0154343_NIC349273_HES_APC_W_201999.txt",
                 sep = "|", stringsAsFactors = FALSE)

HES5 <- read.csv("C:/Users/aadamson/Documents/COPD/SC_outcomes_2019-20/Data/rawData/FILE0154344_NIC349273_HES_APC_W_202099.txt",
                 sep = "|", stringsAsFactors = FALSE)

# write.csv(HES2, "D:/Alex/COPD/SC Outcomes 2018/data/rawData/viewing/HES2.csv", row.names = FALSE) 


ONS1 <- read.csv("C:/Users/aadamson/Documents/COPD/SC_outcomes_2019-20/Data/rawData/FILE0154345_NIC349273_CIVREG_DEATHS_E.txt", 
                    sep = "|", stringsAsFactors = FALSE)

ONS2 <- read.csv("C:/Users/aadamson/Documents/COPD/SC_outcomes_2019-20/Data/rawData/FILE0154346_NIC349273_CIVREG_DEATHS_W.txt", 
                 sep = "|", stringsAsFactors = FALSE)



ONS3 <- read.csv("C:/Users/aadamson/Documents/COPD/SC_outcomes_2019-20/Data/rawData/FILE0154346_NIC349273_CIVREG_DEATHS_W.txt", 
                 sep = "|", stringsAsFactors = FALSE)
...
dat %>% select(patientid) %>% unique() %>% filter(patientid %in% ONS4$study_id) %>% nrow() # only study_id matches
dat %>% select(studyid) %>% unique() %>% filter(studyid %in% ONS4$study_id) %>% nrow()


# An extra ONS to come...

# ONS4 <- read.csv("C:/Users/aadamson/Documents/COPD/SC_outcomes_2018-19/Data/rawData/FILE0157734_NIC357479_CIVREG_MORT_WALES_ADDITIONAL.txt", 
#                  sep = "|", stringsAsFactors = FALSE)

# ONS4 doesn't match up with anyone and the format is stupid anyway, so we just leave it. 


# ONS <- left_join(ONS1, ONS2, by = "ID")

ONS <- bind_rows(ONS1, ONS2, ONS3)
...
NRS <- read.csv("C:/Users/aadamson/Documents/COPD/SC_outcomes_2019-20/Data/rawData/nrsdeaths-COPD-201910-202002-v202.csv", 
                    sep = ",", stringsAsFactors = FALSE)
...
SMR <- read.csv("C:/Users/aadamson/Documents/COPD/SC_outcomes_2019-20/Data/rawData/smr01-COPD-201910-202002-v202.csv", 
                    sep = ",", stringsAsFactors = FALSE)
...
nlc("Total number of HES admitting episodes:") # ideal situation: 201234 admissions, but all the discharge
                                               # data is kept in the final episode only
...
summary(dat$q1_2aarrivaldate)
summary(dat$q4_1aadmissiondate)

summary(dat$q10_2dischargedate)

head(sort(dat$q4_1aadmissiondate))
...
PEDW <- read.csv("C:/Users/aadamson/Documents/COPD/SC_outcomes_2019-20/Data/rawData/Request_31488_NACAP_COPD_Cohort2.csv")
...
datboth %>% group_by(patientid) %>% add_tally(wt = indexadmission) %>% slice(1) %>% ungroup() %>% select(n) %>% table() 
#  select(patientid, AdmissionDate, DischargeDate, IgnoreReadmit, matchtype, indexadmission) %>% print(n=200)
...
datboth$IMD_combined[is.na(datboth$IMD_combined)] <- paste("Scot", datboth$simd2020v2quin[is.na(datboth$IMD_combined)], sep = "_") 
