...
dat2 <- readRDS(
"C:/Users/aadamson/Documents/COPD/SC_outcomes_2019-20/Data/tidyData/linked_audit_HES_PEDW_ONS_data_2019-20.RDS")

dat <- bind_rows(dat1, dat2)

dat1 %>% filter(indexadmission == 1) %>% nrow()
dat2 %>% filter(indexadmission == 1) %>% nrow()
...
dat_general <- dat %>% group_by(hosp_code) %>% slice(1) %>% select(hosp_code, hosp_name, trust_code, trust_name, region, country)
...
dat30died <- dat %>%
  ...
  rename(COD30diagcount = n) %>% arrange(hosp_code, desc(COD30diagcount)) %>% slice(1:5) %>%
  rename(COD30_diag_code = CAUSE_OF_DEATH_conv) %>% mutate(CODrank = paste0("highest_diag_", row_number())) %>%
...
# write.csv(all_dat, "C:/Users/aadamson/Documents/COPD/SC_outcomes_2018-19/Data/tidyData/COPD_SC_outcomes_2018-20_data.csv",
#         row.names = FALSE)
...
datalive$LOSfac <- factor(datalive$LOS)
dat$LOSfac <- factor(dat$LOS)
...
died30COPD_mod <- glmer(died30flag ~ 1 + gender + new_IMD_quintile + agecat + CCIweightedcat + q7_1acutetreatmentwithniv +
              (1 | hosp_code), family=binomial(link = "logit"), data = dat,
            glmerControl(optimizer = "Nelder_Mead"))


died30COPD_mod_fixed <- glm(died30flag ~ 1 + gender + new_IMD_quintile + agecat + CCIweightedcat + q7_1acutetreatmentwithniv,
                            family=binomial(link = "logit"), data = dat)
                        
Sys.time()

summary(died30COPD_mod)


saveRDS(died30COPD_mod, "C:/Users/aadamson/Documents/COPD/SC_outcomes_2019-20/Data/tidyData/died30COPD_mod.RDS")
...
died90COPD_mod <- glmer(died90flag ~ 1 + gender + new_IMD_quintile + agecat + CCIweightedcat + q7_1acutetreatmentwithniv +
                          (1 | hosp_code), family=binomial(link = "logit"), data = dat,
                        glmerControl(optimizer = "Nelder_Mead"))


died90COPD_mod_fixed <- glm(died90flag ~ 1 + gender + new_IMD_quintile + agecat + CCIweightedcat + q7_1acutetreatmentwithniv,
                            family=binomial(link = "logit"), data = dat)
Sys.time()



saveRDS(died90COPD_mod, "C:/Users/aadamson/Documents/COPD/SC_outcomes_2019-20/Data/tidyData/died90COPD_mod.RDS")


read30COPD_mod <- glmer(read30flag ~ 1 + gender + new_IMD_quintile + agecat + CCIweightedcat + q7_1acutetreatmentwithniv + LOS + 
                          (1 | hosp_code), family=binomial(link = "logit"), data = datalive,
                        glmerControl(optimizer = "Nelder_Mead"))

summary(read30COPD_mod)


read30COPD_mod_fixed <- glm(read30flag ~ 1 + gender + new_IMD_quintile + agecat + CCIweightedcat + q7_1acutetreatmentwithniv + LOS, 
...
saveRDS(read30COPD_mod, "C:/Users/aadamson/Documents/COPD/SC_outcomes_2019-20/Data/tidyData/read30COPD_mod.RDS")


read90COPD_mod <- glmer(read90flag ~ 1 + gender + new_IMD_quintile + agecat + CCIweightedcat + q7_1acutetreatmentwithniv + LOS + 
                          (1 | hosp_code), family=binomial(link = "logit"), data = datalive,
                        glmerControl(optimizer = "Nelder_Mead"))

Sys.time()

read90COPD_mod_fixed <- glm(read90flag ~ 1 + gender + new_IMD_quintile + agecat + CCIweightedcat + q7_1acutetreatmentwithniv + LOS, 
                          family=binomial(link = "logit"), data = datalive)
                      


saveRDS(read90COPD_mod, "C:/Users/aadamson/Documents/COPD/SC_outcomes_2019-20/Data/tidyData/read90COPD_mod.RDS")
...
di30_COPD <- tidyoutput(died30COPD_mod, meth = "Wald") %>% furtherCleaning() %>% rename(`Estimates: Died in 30 days` = Estimate)
di90_COPD <- tidyoutput(died90COPD_mod, meth = "Wald") %>% furtherCleaning() %>% rename(`Estimates: Died in 90 days` = Estimate)
re30_COPD <- tidyoutput(read30COPD_mod, meth = "Wald") %>% furtherCleaning() %>% rename(`Estimates: Readmitted in 30 days` = Estimate)
re90_COPD <- tidyoutput(read90COPD_mod, meth = "Wald") %>% furtherCleaning() %>% rename(`Estimates: Readmitted in 90 days` = Estimate)

di30_COPD_fixed <- tidyoutput(died30COPD_mod_fixed, meth = "Wald", MEM = FALSE) %>% furtherCleaning() %>% rename(`Estimates: Died in 30 days` = Estimate)
di90_COPD_fixed <- tidyoutput(died90COPD_mod_fixed, meth = "Wald", MEM = FALSE) %>% furtherCleaning() %>% rename(`Estimates: Died in 90 days` = Estimate)
re30_COPD_fixed <- tidyoutput(read30COPD_mod_fixed, meth = "Wald", MEM = FALSE) %>% furtherCleaning() %>% rename(`Estimates: Readmitted in 30 days` = Estimate)
re90_COPD_fixed <- tidyoutput(read90COPD_mod_fixed, meth = "Wald", MEM = FALSE) %>% furtherCleaning() %>% rename(`Estimates: Readmitted in 90 days` = Estimate)
...
all <- di30 %>% left_join(., di90, by = "Variable") %>% left_join(., re30, by = "Variable") %>% left_join(., re90, by = "Variable")

all <- all %>% filter(!(Variable %in% c("(Intercept)", "genderTransgender", "genderOther", "genderNot recorded/Preferred not to say",
                                        "IMD_combinedMissing")))
                                        

# write.csv(all, "C:/Users/aadamson/Documents/COPD/SC_outcomes_2018-19/Data/dataStore/COPD_regression_results.csv")
